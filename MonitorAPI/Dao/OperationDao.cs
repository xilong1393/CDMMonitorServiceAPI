...
namespace MonitorAPI.Dao
{
    public class OperationDao:BaseDao
    {
        ...
        private const string SQL_SCHEDULE_DAY_FILTEER = " and CLASSRECORDING.CLASSSTARTTIME >= '{0} 00:00:00' and CLASSRECORDING.CLASSSTARTTIME <= '{0} 23:59:59'";
        ...
        internal List<ClassRecordingWithSchedule> GetClassRecordingbyClassroomIDAndDate(int classroomID, DateTime dateTime)
        {
            using (SqlCommand command = new SqlCommand())
            {
                command.Connection = Connection;
                ...
                string sqlstr = SQL_GETCLASSRECORDING_CLASSROOMID + string.Format(SQL_SCHEDULE_DAY_FILTEER, datestring);

                AddParamToSQLCmd(command, "@CLASSROOMID", SqlDbType.Int, 0, ParameterDirection.Input, classroomID);
                SetCommandContent(command, CommandType.Text, sqlstr);
                return SqlHelper.ExecuteReaderCmdList<ClassRecordingWithSchedule>(command);
            }
        }
