...
{
    public sealed class ClassroomService : BaseService
    {
        ...
        public DataTable GetClassroomByGroupID(string sessionID, int groupID, DateTime dateTime)
        {
            using (PersistenceContext pc = new PersistenceContext())
            {
                ClassroomDao classroomDao = new ClassroomDao(pc);
                DataTable dt = CreateDataTable(classroomDao.GetClassroomTableByGroupID(groupID));
                return dt;
            }
        }
