...
namespace MonitorAPI.Controllers
{
    public class ClassroomController : ApiController
    {
        ...
        [HttpGet]
        public IHttpActionResult GetClassroomGroupListByRoleID(int roleID)
        {
            try
            {
                ClassroomGroupService service = ServiceFactory.ClassroomGroupService;
                List<ClassroomGroup> list = service.GetClassroomGroupListByRoleID(roleID);
                return Ok(list);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public IHttpActionResult GetClassroomGroupbyClassroomID(ClassroomGroupbyClassroomID cgcID)
        {
            try
            {
                ClassroomGroupService service = ServiceFactory.ClassroomGroupService;
                ClassroomGroup classroomGroup = service.GetClassroomGroupbyClassroomID(cgcID.ClassroomID);
                return Ok(classroomGroup);
            }
            catch (Exception ex)
            {
                LogHelper.GetLogger().Error(ex.ToString());
